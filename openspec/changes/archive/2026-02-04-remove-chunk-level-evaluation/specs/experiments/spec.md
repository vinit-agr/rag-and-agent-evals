...
The system SHALL define `ExperimentConfig` as a single interface (not a discriminated union) with fields `name: string`, `corpus: Corpus`, `retriever: Retriever`, `k: number`, `groundTruth: GroundTruth[]`, and optional `metrics: Metric[]`. The `evaluationType` discriminator field SHALL NOT exist.
...
#### Scenario: No evaluationType field
- **WHEN** creating an experiment config
- **THEN** there SHALL be no `evaluationType` field to set
...
**Reason**: With only one evaluation type, the discriminated union is unnecessary complexity.
**Migration**: Use the simplified `ExperimentConfig` interface with `groundTruth: GroundTruth[]`.

### Requirement: Default metrics based on evaluation type
**Reason**: With only one evaluation type, metrics selection based on type is unnecessary.
**Migration**: Default metrics are always `[recall, precision, iou]`.

## RENAMED Requirements

### Requirement: ExperimentConfig type
- **FROM**: `TokenLevelExperimentConfig`
- **TO**: `ExperimentConfig`
